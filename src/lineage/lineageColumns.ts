...
export async function resolveLineageColumns(
    ...
    const fromCatalog = (n: LineageNode): Promise<Column[]> => {
        const table = tableOf(n.fullName, defaultDatabase);
        if (!table) { return Promise.reject(new Error('no database: set one on the active connection')); }
        const key = `${table.database}.${table.schema}.${table.table}`.toLowerCase();
...
/** `db.schema.table`, `schema.table` or `table` against the active connection's database. */
export function tableOf(fullName: string, defaultDatabase?: string): ResolvedTable | null {
    const parts = fullName.split('.').map(p => p.replace(/^\[|\]$/g, ''));
    const [database, schema, table] =
        parts.length >= 3 ? parts.slice(-3)
            : parts.length === 2 ? [defaultDatabase, parts[0], parts[1]]
                : [defaultDatabase, 'dbo', parts[0]];
    return database ? { database, schema: schema!, table: table! } : null;
}
