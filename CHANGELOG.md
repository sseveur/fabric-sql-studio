...
- **T-SQL language services** - Completion (keywords, functions, `alias.` and three-part column lists from `INFORMATION_SCHEMA`), hovers for tables and CTEs with their columns and types, semantic tokens, folding, snippets and a `sql-formatter` transactsql formatter with style options.
...
- **Data lineage** - Sources, CTEs and targets (`INSERT`, `SELECT INTO`, `CREATE VIEW`, `MERGE`, `UPDATE`, `DELETE`) for the file or a selection, drawn as a layered graph: edges that skip steps route around boxes, crossings are minimised and every edge has its own arrow. Hovering a box highlights its upstream and downstream; clicking it opens the SQL at that line. A **Columns** toggle lists each table's, CTE's and result's columns with types (sources and targets from the catalog, CTEs from their SELECT lists). Exportable as PNG or PDF.
